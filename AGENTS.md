...
- **Vercel deploys the public site from GitHub `main`.** The Vercel project `owner-vercel-team/openvoiceflow` is connected to `shimoverse/openvoiceflow` with production branch `main`. Root `vercel.json` runs `npm run vercel-build`, which copies `docs/` into `public/` for the deployed static site.

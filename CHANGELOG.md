...
### Fixed
- **Automatic updates now actually install.** Sparkle downloaded new builds in the background but only installed them when the app quit — and a menu-bar app never quits, so the update sat staged until you clicked **Check for Updates…** yourself. A downloaded, verified update now installs and relaunches on its own the moment no dictation is in flight. The daily check also moved from "24 hours after the last one" to a fixed 3:00 PM Pacific; a Mac that was asleep or off at 3 PM checks as soon as it is back. **Automatic updates** in Settings still turns all of this off.

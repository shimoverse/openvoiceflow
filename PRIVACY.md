...
**Rating the app (since v0.5.27).** Every so often — after a couple of weeks of use, and then no more than once a season — a small card in the corner of the dashboard asks how OpenVoiceFlow is treating you: one to five stars, and an optional comment. Nothing is sent until you press **Send**; dismissing the card sends nothing and snoozes it. What goes out is the star rating, your comment as you typed it (capped at 1,000 characters; the one free-text field the app ever uploads), the app version, and your country derived server-side from the request. While "Share anonymous usage & leaderboard rank" is on, the same anonymous device ID as the leaderboard rides along so we can read a rating next to that installation's version and usage counters; with it off, the rating arrives with no device ID at all. Ratings and comments are read only by the maintainers, never shown publicly, and never include dictated text, snippets, dictionary entries, or Know-Me profile content — only what you chose to write in the box. "Delete my leaderboard data" also removes the ratings tied to that device ID.

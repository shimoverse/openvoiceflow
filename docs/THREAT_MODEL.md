...
7. **DMG signing / notarization.** The current v0.3.5 website-hosted DMGs are signed with Developer ID, notarized by Apple, and stapled so macOS Gatekeeper can verify them. The release workflow supports the same path when Apple Developer credentials are configured.
